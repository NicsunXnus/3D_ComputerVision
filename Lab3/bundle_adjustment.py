...
def compute_ba_residuals(parameters: np.ndarray, intrinsics: np.ndarray, num_cameras: int, points2d: np.ndarray,
    ...
    """
    proj_matrixes = np.matmul(intrinsics, extrinsics)
    points3d = np.concatenate([points3d, np.ones((points3d.shape[0], 1))], axis=1)
    reproj_points2d = np.matmul(proj_matrixes, points3d[points3d_idxs].T)
    euc_dist = np.sqrt(np.square(reproj_points2d[:, 0]/reproj_points2d[:, -1] - points2d.T[0]) + np.square(reproj_points2d[:, 1]/reproj_points2d[:, -1] - points2d.T[1]))
    residuals = np.squeeze(np.sum(euc_dist, axis=0))
    """ END YOUR CODE HERE """
    return residuals
