...
NOTES:
--removed underscores before KeyPoint parameters due to different cv2 versions
...
def create_feature_matches(image_file1: os.path, image_file2: os.path, lowe_ratio: float = 0.6, min_matches: int = 10):
    ...
    """
    # cv.BFMatcher() - brute force matcher which returns 1 best match
    # knnMatch - returns k best matches
    # Run cv.BFMatcher() and matcher.knnMatch(descriptors1, descriptors2, 2)
    bf = cv2.BFMatcher_create()
    matches = bf.knnMatch(descriptors1, descriptors2, 2)
    # Filter the feature matches using the Lowe ratio test.
    # Good matches are when the first match has distance less than the
    #    <lowe_ratio> x distance of the second best match.
    for m,n in matches:
...
def create_ransac_matches(image_file1: os.path, image_file2: os.path,
    ...
    """
    essential_mtx, is_inlier = cv2.findEssentialMat(points1=points1, points2=points2, cameraMatrix=camera_intrinsics, method=cv2.RANSAC, prob=0.999,threshold=ransac_threshold)
    """ END YOUR CODE HERE """
...
def create_scene_graph(image_files: list, min_num_inliers: int = 40):
    ...
    <min_num_inliers> 
    """
    for ind_i, img_id_i in enumerate(image_ids):
        for ind_j, img_id_j in enumerate(image_ids):
            if (img_id_i != img_id_j):
                # Get the number of inliers between each images
                np_arr = os.path.join(RANSAC_MATCH_DIR, f"{img_id_i}_{img_id_j}.npy")
                if os.path.exists(np_arr) and len(np.load(np_arr)) >= min_num_inliers:
                    graph.add_edge(ind_i, ind_j) #connect this two images if satisfied
    """ END YOUR CODE HERE """
