...
def get_init_image_ids(scene_graph: dict) -> (str, str):
    ...
    """ YOUR CODE HERE """
    max_inliers = 0
    for img_id, neighbors in scene_graph.items():
        for neighbor in neighbors:
            matches = load_matches(img_id, neighbor)
            inliers = matches.shape[0]
            #pick the image pair with the largest number of inliers
            if inliers > max_inliers:
                max_pair = [img_id, neighbor]
                max_inliers = inliers
...
def get_init_extrinsics(image_id1: str, image_id2: str, intrinsics: np.ndarray) -> (np.ndarray, np.ndarray):
    ...
    _,R,t,mask = cv2.recoverPose(E=essential_mtx, points1=points2d_1,points2=points2d_2, cameraMatrix=intrinsics, R=R, t=t, mask=mask)
...
def get_reprojection_residuals(points2d: np.ndarray, points3d: np.ndarray, intrinsics: np.ndarray,
    ...
    homography = np.dot(intrinsics, np.hstack([rotation_mtx, tvec.reshape(3, 1)]))
    
    for i in range(len(points3d)):
        pt_3d = points3d[i]
        pt_3d = np.vstack([pt_3d.reshape(3, 1), [1]])
        reprojected_pt2d = np.dot(homography, pt_3d)
        reprojected_pt2d = reprojected_pt2d / reprojected_pt2d[-1]
        point1 = points2d[i]
        point2 = reprojected_pt2d[:-1]
        euclid_dist = np.sqrt((point2[0]-point1[0])**2 + (point2[1]-point1[1])**2)
        residuals[i] = euclid_dist
    """ END YOUR CODE HERE """
    return residuals
...
def solve_pnp(image_id: str, point2d_idxs: np.ndarray, all_points3d: np.ndarray, point3d_idxs: np.ndarray,
    ...
    for _ in range(num_ransac_iterations):
        ...
        """
        rvec = None
        _, rvec, tvec = cv2.solvePnP(selected_pts3d, selected_pts2d, intrinsics, None, rvec, tvec, flags=cv2.SOLVEPNP_ITERATIVE)
...
def get_next_pair(scene_graph: dict, registered_ids: list):
    ...
    for registered_id in registered_ids:
        unregistered_neighbors = [neighbor_id for neighbor_id in scene_graph[registered_id] if neighbor_id not in registered_ids]
        for next_id in unregistered_neighbors:
            matches = load_matches(image_id1=registered_id, image_id2=next_id)
            num_inliers = matches.shape[0]
            if num_inliers > max_num_inliers:
                max_registered_id, max_new_id = registered_id, next_id
                max_num_inliers = num_inliers
    """ END YOUR CODE HERE """
