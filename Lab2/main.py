...
def detect_lines(image: np.ndarray) -> np.ndarray:
    ...
    # 2. Pass the edge image from Canny edge detection to cv2.HoughLinesP with rho=1, theta=np.pi/180, threshold=100,
    #    minLineLength=MIN_LINE_LENGTH and maxLineGap=MAX_LINE_GAP.
    line_pts = cv2.HoughLinesP(edges, rho=1, theta=np.pi/180, threshold=100,
...
def get_pairwise_intersections(lines: np.ndarray) -> np.ndarray:
    ...
    for i in range(num_lines):
        for j in range(i + 1,num_lines):
            ...
            poi = np.cross(lineA,lineB)
            k = poi[2]
            if (k != 0): # points of infinities have k = 0, etc; (x,y,k=0)
                intersections[ind] = poi/k
                ind+=1
    """ END YOUR CODE HERE """
...
def get_support_mtx(intersections: np.ndarray, lines: np.ndarray, distance_threshold: float = DISTANCE_THRESHOLD):
    ...
    """
    distance_mtx = np.empty(shape=[num_intersections, num_lines], dtype=intersections.dtype)
    # 1. Compute the I x L distance matrix distance_mtx between lines and intersections.
    for i in range(num_intersections):
        for j in range(num_lines):
            u, v = intersections[i][0], intersections[i][1]
            a, b, c = lines[j]
            # Calculate the distance between the line and intersection point
            distance = np.abs(a * u + b * v + c) / np.sqrt(a**2 + b**2)
            distance_mtx[i, j] = distance
            
    # 2. Set the support matrix [i, j] to 1 if the distance_mtx[i,j] is below the distance threshold.
    support_mtx = (distance_mtx < distance_threshold)
    """ END YOUR CODE HERE """
...
def get_vanishing_pts(lines: np.ndarray, num_vanishing_pts: int) -> np.ndarray:
    ...
    for _ in range(num_vanishing_pts):
        # Find the intersection with the most support as the vanishing point
        vanishing_point_idx = np.argmax(np.sum(support_mtx, axis=1))
        vanishing_point = intersections[vanishing_point_idx]
        vanishing_pts.append(vanishing_point)
        # Remove its support set (of lines) from the list of lines
        supporting_lines = np.nonzero(support_mtx[vanishing_point_idx])[0]
        support_mtx[:, supporting_lines] = 0
    """ END YOUR CODE HERE """
...
def get_vanishing_line(vanishing_pts: np.ndarray):
    ...
    """ YOUR CODE HERE """
    #Obtain vanishing line through cross product of two vp
    ptA = vanishing_pts[0]
    ptB = vanishing_pts[1]
    vanishing_line = np.cross(ptA,ptB)
    """ END YOUR CODE HERE """
...
def get_target_height(vanishing_line: np.ndarray, query_info: dict, target_info: dict, vanishing_pt_v: np.ndarray):
    ...
    """
    # 1. vanishing point u -> Compute the vanishing point 𝐮 = (𝐛1 × 𝐛2) × 𝐥
    vanishing_point_u = np.cross(np.cross(pt_b1, pt_b2),vanishing_line)
    # 2. transferred point -> Compute the transferred point t = (𝐭1 × 𝐮) × 𝐥2, where 𝐥2 = 𝐯 × 𝐛2.
    transferred_point = np.cross(np.cross(pt_t1,vanishing_point_u),np.cross(pt_b2,vanishing_pt_v))
    #Division by homoegenous coordinate
    vanishing_point_u = vanishing_point_u/vanishing_point_u[2]
    transferred_point = transferred_point/transferred_point[2]
    # 3. distances of scene points wrt to point b2
    t1 = np.sqrt((transferred_point[0] - pt_b2[0])**2 + (transferred_point[1] - pt_b2[1])**2)
    t2 = np.sqrt((pt_t2[0] - pt_b2[0])**2+(pt_t2[1] - pt_b2[1])**2)
    v = np.sqrt((vanishing_pt_v[0] - pt_b2[0])
                     ** 2 + (vanishing_pt_v[1] - pt_b2[1])**2)
    # 4. distance ratio - Reference to page 77 Lecture 5
    distance_ratio = (t1 * (v - t2)) / (t2 * (v - t1))
    # 5. target height from distance ratio
    target_height = query_d1 / distance_ratio
    """ END YOUR CODE HERE """
